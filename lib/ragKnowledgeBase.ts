...
export interface RAGDocument {
  ...
  sourceRole?: 'publication' | 'thesis' | 'web' | 'other'
  metadata?: {
    title?: string
    year?: string
    venue?: string
...
export function useRAGKnowledgeBase() {
  ...
  const crawlSite = async (ragId: string, url: string, options?: CrawlOptions) => {
    setLoading(true)
    setError(null)

    const result = await crawlWebsite(ragId, url, options)
