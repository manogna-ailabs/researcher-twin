/**
 * Global Fetch Interceptor for local agent API calls
 *
...
// Local agent endpoint to intercept
const LYZR_API_URL = '/api/agent'
...
export function sendFixRequestToParent(): void {
  ...
  try {
    ...
    console.debug('[AgentInterceptor] Fix request sent to parent (user clicked)')
    clearPendingError()
  } catch (e) {
...
async function interceptedFetch(
  ...
  // Only intercept local agent API calls
  if (!url.includes('/api/agent')) {
    return originalFetch(input, init)
  }

  console.debug('[AgentInterceptor] Intercepting local agent API call')
...
export function installAgentInterceptor(): void {
  ...
  console.debug('[AgentInterceptor] Installed global fetch interceptor for local agent API')
}
...
export function uninstallAgentInterceptor(): void {
  ...
  console.debug('[AgentInterceptor] Uninstalled fetch interceptor')
}
