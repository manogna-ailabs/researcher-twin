...
export const sendErrorToParent = (error: ErrorDetails): void => {
  if (!isInIframe()) {
    console.debug('[ErrorBoundary] Not in iframe, skipping parent notification')
    return
  }
  ...
  try {
    ...
    console.debug('[ErrorBoundary] Error sent to parent:', error.type)
  } catch (e) {
...
export const requestFixFromParent = (error: ErrorDetails): void => {
  if (!isInIframe()) {
    console.debug('[ErrorBoundary] Not in iframe, cannot request fix')
    return
  }
  ...
  try {
    ...
    console.debug('[ErrorBoundary] Fix request sent to parent')
  } catch (e) {
    ...
    const handleError = (event: ErrorEvent) => {
      ...
      if (isInIframe()) {
        ...
        if (errorType === 'react_error') {
          console.debug('[ErrorBoundary] Code error detected, auto-requesting fix...')
    ...
    const handleConsoleError = (...args: unknown[]) => {
      ...
      if (isCodeError && isInIframe()) {
        ...
        if (lastErrorKey !== errorKey) {
          ...
          console.debug('[ErrorBoundary] Console error detected, sending to parent...')
          sendErrorToParent(errorDetails)
          requestFixFromParent(errorDetails)
          setError(errorDetails)
        }
